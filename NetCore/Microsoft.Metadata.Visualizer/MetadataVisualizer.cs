...
namespace Microsoft.Metadata.Visualizer
{
    ...
    */
    public sealed class MetadataVisualizer
    {
        ...
        private TEntity Get<TEntity>(EntityHandle handle, Func<MetadataReader, Handle, TEntity> getter)
        {
            if (aggregator != null)
            {
                int generation;
        ...
        public string Token(EntityHandle handle, bool displayTable = true)
        {
            if (handle.IsNil)
            {
                return "nil";
        ...
        private void WriteTypeDef()
        {
            ...
            foreach (var handle in reader.TypeDefinitions)
            {
                ...
                int size, packingSize;

                TypeLayout layout = entry.GetLayout();
                bool hasLayout = layout.IsDefault;
        ...
        private void WriteEnCMap()
        {
            ...
            foreach (var entry in reader.GetEditAndContinueMapEntries())
            {
                if (aggregator != null)
                {
                    ...
                    EntityHandle eh = (EntityHandle)primary;

                    AddRow(
                        Token(entry),
                        primaryModule.Generation.ToString(),
                        "0x" + MetadataTokens.GetRowNumber(eh).ToString("x6"),
                        isUpdate ? "update" : "add");
                }
                else
                {
